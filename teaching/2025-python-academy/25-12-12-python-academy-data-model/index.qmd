...
Python's built-in types just... work. `len()` for sizes, `[]` for lookups, `for` loops for iteration. It all feels consistent because it is.

That consistency comes from the **Python Data Model**. By implementing a few "special methods" (you might know them as *dunder* methods, short for **d**ouble **under**score), you can make your own custom classes behave exactly like the built-ins.
...
We don't want to scan through a list to find a stock — that's O(N). The `__getitem__` method allows us to use the square bracket notation `[]` to access items directly.
...
We can already check membership via `__iter__`, but that's O(N). Since we have a dictionary underneath, `__contains__` gives us O(1) lookup.
...
These patterns are what make Python code feel like Python. In the [next post](../25-12-18-python-academy-oop-basics/), we'll use inheritance and protocols to build a yield curve hierarchy.

::: callout-note

Download the whole code [here](../../../assets/python/python_data_model_portfolio_implementation_example.py).
