...
This "functional programming" trait of Python is a core for some powerful patterns that wouldn't work in some other (purely OOP) languages.
...
Functions are more than callable blocks of code — and that flexibility is what makes closures and decorators possible.
...
Closures are a key ingredient for decorators, which we'll get to next.
They also let us create "function factories" that can produce specialized functions.
...
1. This is a cleaner way of writing `fibonacci_cached_by_me = my_cache(fibonacci_cached_by_me)`. A decorator is just a function that takes another function as input and returns a modified function.
...
Decorators show up everywhere — Flask uses them for routing, `pytest` for fixtures. In the [next post](../25-11-05-python-academy-iteration/), we'll use one to profile memory usage of iterators and generators.

::: callout-note

Download the whole code [here](../../../assets/python/python_functions_closures_and_decorators_explained.py).
