...
Fibonacci numbers are a classic interview question and a great way to talk about
optimization. In this post, we're going to see how many different ways there are to
solve the same problem in Python—from the "obvious" way that's painfully slow, to a few
clever tricks that make it fast.
...
a noticeable amount of time to compute. The reason for this is that the function calls
itself twice for each `n`, leading to an exponential growth of the call stack.
...
As you can see, many values are calculated multiple times. For example,
`fibonacci_recursive(4)` is calculated twice, `fibonacci_recursive(3)` is calculated
three times, and so on. This is the reason for the inefficiency.

## Memoization

We can improve the performance by storing the results of the function calls in a cache
(a dictionary). This technique is called memoization.
...
This is the most efficient way (we'll see) to calculate Fibonacci numbers in Python. It
has a linear time complexity and constant space complexity.
...
1. Force compilation
1. This is the line that cannot be compiled by Numba in nopython mode
...
[here](../../../assets/python/fibonacci_optimization_and_memoization_case_study.py).
