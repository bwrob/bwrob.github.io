...
In the finance world, we're always neck-deep in data. It's not just about how fast you can process it, but whether your code is going to crash your server by eating up all the memory.

This post is about iterators and generators — the "lazy" way to handle data that saves memory and avoids headaches.
...
An iterator is an object that represents a stream of data. It produces one item at a time, only when requested. The iterator protocol consists of two methods:
...
Generators are a simpler way to create iterators. Instead of writing a class with `__iter__` and `__next__`, you write a function that uses `yield`.
...
The output speaks for itself: creating the generator uses almost no memory. The cost only shows up when you actually consume it into a list.
...
The `itertools` module is worth knowing well. Here are the ones I reach for most in financial work.
...
The iterator protocol (`__iter__` and `__next__`) we covered here is actually our first taste of Python's data model — a topic we'll dig into after covering [class anatomy](../25-12-04-python-academy-class-anatomy/).
