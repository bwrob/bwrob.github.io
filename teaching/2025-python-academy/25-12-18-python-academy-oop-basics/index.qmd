...
We've already seen what goes into a single class, but in a real library, objects don't exist in a vacuum. They need to talk to each other and share common ground. This is where things like **Inheritance** and **Polymorphism** come in.

Think of it this way: a **European Option** *is a* **Financial Instrument**. A **Nelson-Siegel Model** *is a* **Yield Curve**. By setting up our code to reflect these relationships, we can write engines that handle any curve we throw at them without needing a massive "if-else" mess.
...
Inheriting from `ABC` prevents anyone from creating a "plain" `YieldCurve`—it's just a template. The `@abstractmethod` decorator is the enforcer: if a subclass doesn't implement `discount_factor`, Python won't let you instantiate it.
...
2. Get base discount factor from the parent curve
3. Apply spread: D_new(t) = D_old(t) * exp(-spread * t)
...
This is flexible. Want to switch to Cubic Splines? Just pass a different interpolator. The curve class itself doesn't need to change.
...
Because we stuck to the `YieldCurve` contract, the pricing function doesn't need to know about any of the complexity above.
...
**Protocols** (structural subtyping) solve this. A Protocol defines a shape: "I don't care who your parents are, as long as you have a `discount_factor` method."
...
::: {.callout-note}
You can find the complete code for this post here: [inheritance_polymorphism_and_protocols_in_python_oop.py](../../../assets/python/inheritance_polymorphism_and_protocols_in_python_oop.py).
:::
