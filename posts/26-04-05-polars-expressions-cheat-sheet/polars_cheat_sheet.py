...
def main():
    ...
    ).clip(-1, 1).cast(pl.Float32).name.suffix("_norm")
    ...
    print(df.with_columns(
        ...
        size_tag = pl.when(pl.col("sepal_length") > 6).then(pl.lit("XL")).otherwise(pl.lit("L")),
    ...
    print(df.group_by("species").agg(pl.col("petal_length").implode()).select(
        "species",
        top_2 = pl.col("petal_length").list.eval(pl.element().sort(descending=True).head(2))
    ))
    ...
    if os.path.exists("iris_temp.csv"): os.remove("iris_temp.csv")
    if os.path.exists("iris_final.csv"): os.remove("iris_final.csv")
