...
1. Force pseudo-terminal allocation (essential for interactive terminal multiplexers)
2. Run the tmux binary on the remote host
3. Attach to session if it already exists; create it if it does not
4. Name of target session (`work`)
...
2. Name the session `<name>`
3. Run detached in the background
4. Execute initial command `<cmd>` upon startup (optional)
...
1. Attach to an existing session (`a` or `attach-session`)
2. Target session name `<name>` (omit `-t` to attach to the most recent session)
3. Detach other connected clients (forces session to resize to your display window)
