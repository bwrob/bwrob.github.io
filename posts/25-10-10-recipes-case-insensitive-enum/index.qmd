...
description: "Implementing case-insensitive enums in Python."
format-links: [html]
image: cover.jpg
...
![Case Insensitive Enum cover](cover.jpg){width="98%" fig-align="center"}

When working with enums in Python, you might want to create an enum that is
case-insensitive. This can be particularly useful when you want to allow users to input
values without worrying about the case. Here's how you can achieve this by subclassing
`StrEnum`. To enforce that all enum members are defined in lowercase, we can use a hook
`__init_subclass__`.

## Implementation

```{python}
#| label: case-insensitive-enum-implementation
...
```{python}
#| label: test-case-insensitive-enum
...
[case_insensitive_str_enum_implementation.py](../../assets/python/case_insensitive_str_enum_implementation.py).
:::
