...
Well, get ready to throw that outdated script out the window! 
...
But the learning curve is steep, the open-source projects often get abandoned and with great power (sudo) comes great responsibility (and sometimes system reinstall).
...
And limited stakes once you (inevitably) remove half of your file system with a bash script.
...
Fortunately, this is "super easy, barely an inconvenience". 
...
Now, open the Powershell with administrative privileges and run:
...
There is an additional benefit we can reap with just a bit of more work --- a fully system-agnostic setup.
...
It just uses differrent default shell and lacks a package manager, as Apple doesn't trust its clients.
...
Before we dive into installing Homebrew, we need to equip our system with the essential tools. 
The build-essential package provides a collection of compilers and libraries necessary for building software from the source code.
...
You can either generate empty file with `0` or go through the interactive setup with `1`.
![](8.png)

Add the brew initialization to `.zshrc` file as well.
This way `brew` will always be by your side!
```{bash}
