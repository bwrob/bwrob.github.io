...
![](../../assets/images/posts/24-05-21-startup-script/win_98.png){width="65%" fig-align="center"}
...
2. A `Task` is a tuple of a generic task `T` and an `int` delay.
3. A `TaskList` is a list of `Task`s. Simple.
4. A `TaskListOptionalDelay` is a list of `Task`s or `T`s. Those are the guys we are going to turn into `TaskLists`.
5. The decorator `with_optional_delay` adds a default delay of 4 seconds to all non-tuple items. Its input is a worker function that already expects a `TaskList`. It fills in the missing delay with the default value and passes it to the decorated function.
...
![](../../assets/images/posts/24-05-21-startup-script/startup_shortcut.png){width="90%" fig-align="center"}
...
Download the startup [script](../../assets/python/windows_python_development_environment_startup_script.py) and example Windows [shortcut](../../assets/images/posts/24-05-21-startup-script/Startup!.lnk).
