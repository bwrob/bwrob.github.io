...
![](../../assets/images/posts/26-07-02-oom-python/image.jpg){width="95%"
fig-align="center"}

Finding memory leaks in Python gets complicated when using compiled extensions like
NumPy, PyO3, or pybind11. When the system Out of Memory (OOM) killer runs, it terminates
the process with a `SIGKILL` without leaving a stack trace.

To debug an OOM, you need to find where the allocation occurred: in a Python object or
inside a C/C++ backend.

**Memray** is a Python memory profiler that can track allocations down to the native
level. Note that it only works on UNIX-based operating systems.

## Setting Memory Limits

If the Linux OOM killer terminates the process, Memray cannot write its profile data to
disk, and the capture is lost.

We can avoid this by setting a soft memory limit using Python's built-in `resource`
module. This forces the interpreter to raise a `MemoryError` before the OS terminates
the process, allowing Memray to save the profile data.
...
- **NumPy Allocation:** The graph will show the call stack going from Python into
  `numpy.core.multiarray.ones` and down to the C allocation function that blew the 1 GB
  limit.
