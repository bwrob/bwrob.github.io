...
def deriv_numba(y, dx):
    """Numba JIT with precalculated inverses."""
    n = len(y)
...
def _deriv_jax_slicing(y, dx):
    """Core JAX computation (JIT compiled)."""
    inv_dx = 1.0 / dx
    inv_2dx = 0.5 / dx

    # JAX arrays are immutable; we must construct arrays and concatenate
...
def deriv_jax(y, dx):
    """Wrapper to force synchronous execution for accurate timing."""
    return _deriv_jax_slicing(y, dx).block_until_ready()


def deriv_scipy(y, dx):
    """SciPy 1D convolution."""
    inv_2dx = 0.5 / dx
...
def run_benchmark() -> None:
    ...
    # Pre-allocate JAX array so we don't benchmark memory transfer overhead
    print("Moving data to JAX...")
    y_jax = jnp.array(y)

    print("Warming up compilers (Numba & JAX)...")
    _ = deriv_numba(np.array([1.0, 2.0, 3.0]), dx)
    _ = deriv_jax(jnp.array([1.0, 2.0, 3.0]), dx)
    ...
    print(f"{'Method':<20} | {'Average Time (s)':<18} | {'Relative Speed'}")
    print("-" * 60)

    for name, func in methods.items():
        # Pass the JAX array to the JAX function, and the NumPy array to the rest
    ...
    for name, avg_time in sorted_results.items():
        ...
        print(f"{name:<20} | {avg_time:.6f} s         | {relative:.2f}x")
