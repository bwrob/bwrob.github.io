...
2.  **Znajdź `pythonw.exe`**:
    * Najczęstsza lokalizacja to folder `Scripts` w katalogu instalacyjnym Pythona. Na przykład: `C:\Users\TwojaNazwaUzytkownika\AppData\Local\Programs\Python\Python39\pythonw.exe` (ścieżka może się różnić w zależności od wersji Pythona i sposobu instalacji).
...
4.  **Wpisz lokalizację elementu**: W polu "Wpisz lokalizację elementu:" wpisz następującą ścieżkę, zastępując odpowiednie fragmenty:
    ...
    *Przykład:* Jeśli `pythonw.exe` jest w `C:\Python\Python39\pythonw.exe`, a gra `moja_gra.py` jest w `D:\Gry\MojaGra\moja_gra.py`, to ścieżka będzie wyglądać tak:
